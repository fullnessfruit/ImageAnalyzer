...
**이미지는 워커가 직접 받는다.** job에는 이미지 바이트가 아니라 URL이 실려 온다 (확장이 자기 문맥으로 `post`·`blocks`도 같이 싣지만 워커는 읽지 않는다 - 브로커가 결과에 복사하는 용도다). pbs.twimg.com은 공개라 노트북이 자기
...
- OCR 자체가 터지면 결과를 올리지 않고 `POST /jobs/<id>/release`로 lease만 놓는다 → 다음
  폴링에서 다시 잡힌다. 결과가 올라와야만 브로커가 job을 지우므로 유실되지 않는다. 브로커는 이
  재시도를 job당 한 번만 허용한다 - 두 번째 release면 브로커가 에러 결과로 job을 닫고, 확장은
  그 포스트를 포기한다
...
- **`409 already-completed`는 에러가 아니다.** 브로커는 job 하나당 결과 하나만 받고, 결과가
  올라오면 job 파일을 지운다. 그래서 lease(기본 30분)가 만료된 사이에 다른 워커가 같은 job을
  잡아 먼저 끝내면, 뒤늦게 끝난 쪽의 업로드는 409로 거부된다. 같은 입력에 대한 중복 답이므로
  버리면 되고, 재시도해도 영원히 409다. 로그만 남기고 **남은 큐를 계속 비운다**(다른 실패처럼
  `return`하지 않는다). 이 구분을 위해 `brokerFetch`는 던지는 에러에 `status`를 실어 준다
  (`BrokerError`) - 메시지 문자열을 파싱해 판정하지 않기 위함이다
- 워커를 여러 대 띄우는 것은 정상 사용법이다. claim은 lease로 분배되고, 위 409가 중복 완료를
  흡수한다. 다만 **브로커 프로세스는 하나여야 한다** - 무락 원자성이 단일 Node 프로세스 전제다
