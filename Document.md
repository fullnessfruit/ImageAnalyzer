...
적으면 안 된다. 실측 근거:
- **face 0.28** — 동일 인물 0.64~0.79, 화질 열화본(사진 속 사진) 0.32, 다른 인물 0.15.
...
### 캐릭터는 실패 지점이 다른 두 신호를 겹친다
- **WD-Tagger**: 세계 지식 기반. 등록 **0장**으로 알려진 캐릭터 이름이 나오고 화풍 변화에 강하다. 마이너 캐릭터에 약하다.
- **CCIP**: 갤러리 기반. 서로 다른 작가·화풍의 같은 캐릭터를 positive로 학습해 동인 일러스트에도 신원이 유지된다. 등록 1~2장이면 되고 태거가 모르는 캐릭터를 잡는다.
...
`recognizeCharacters()`는 인물이 검출되지 않으면 이미지 전체를 하나의 영역으로 본다.

### server/src/db.ts
**역할**: 임베딩 저장소. 단일 `embeddings` 테이블 — `kind`(face/character/costume) ×
`space`(arcface/ccip/wdtag) × `name` × `image_path`(kind별 UNIQUE) × `embedding` × `auto`.
